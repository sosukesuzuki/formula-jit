...
void crc32(const Reg32e& r, const Operand& op)
{
...
void movq(const Mmx& mmx, const Operand& op)
{
    if (mmx.isXMM())
        db(0xF3);
    opRO(mmx, op, T_0F, mmx.isXMM() ? 0x7E : 0x6F, mmx.getKind() == op.getKind());
}
...
void vpextrw(const Operand& op, const Xmm& x, uint8_t imm)
{
    if (!((op.isREG(16 | i32e) || op.isMEM()) && x.isXMM()))
        XBYAK_THROW(ERR_BAD_COMBINATION)
        if (op.isREG() && x.getIdx() < 16) { opAVX_X_X_XM(Xmm(op.getIdx()), xm0, x, T_0F | T_66, 0xC5, imm); }
    else {
...
void xor (const Operand& op1, const Operand& op2) { xor_(op1, op2); } void xor (const Operand& op, uint32_t imm) { xor_(op, imm); } void not(const Operand& op) { not_(op); }
#endif
...
{
    uint64_t type = (T_N8 | T_F2 | T_0F | T_ER_X | T_MUST_EVEX) | (r.isREG(64) ? T_EW1 : T_EW0);
...
{
    uint64_t type = (T_N2 | T_F3 | T_MAP5 | T_ER_X | T_MUST_EVEX) | (r.isREG(64) ? T_EW1 : T_EW0);
...
void vcvtsi2sh(const Xmm& x1, const Xmm& x2, const Operand& op)
{
    if (!(x1.isXMM() && x2.isXMM() && op.isBit(32 | 64)))
        XBYAK_THROW(ERR_BAD_COMBINATION)
        uint64_t type = (T_F3 | T_MAP5 | T_ER_R | T_MUST_EVEX | T_M_K) | (op.isBit(32) ? (T_EW0 | T_N4) : (T_EW1 | T_N8));
...
{
    uint64_t type = (T_N4 | T_F3 | T_0F | T_ER_X | T_MUST_EVEX) | (r.isREG(64) ? T_EW1 : T_EW0);
    opVex(r, &xm0, op, type, 0x79);
}
...
{
    uint64_t type = (T_N8 | T_F2 | T_0F | T_SAE_X | T_MUST_EVEX) | (r.isREG(64) ? T_EW1 : T_EW0);
    opVex(r, &xm0, op, type, 0x78);
}
void vcvttsh2si(const Reg32e& r, const Operand& op)
{
    uint64_t type = (T_N2 | T_F3 | T_MAP5 | T_EW0 | T_SAE_X | T_MUST_EVEX) | (r.isREG(64) ? T_EW1 : T_EW0);
...
{
    uint64_t type = (T_N4 | T_F3 | T_0F | T_SAE_X | T_MUST_EVEX) | (r.isREG(64) ? T_EW1 : T_EW0);
    opVex(r, &xm0, op, type, 0x78);
}
