...
#if ((defined(_MSC_VER) && (_MSC_VER >= 1600)) || defined(_LIBCPP_VERSION) || ((__cplusplus >= 201103) || defined(__GXX_EXPERIMENTAL_CXX0X__)))
...
class CodeArray {
    ...
    struct AddrInfo {
        ...
        {
            uint64_t disp = (mode == inner::LaddTop) ? jmpAddr + size_t(top) : (mode == inner::LasIs) ? jmpAddr
                                                                                                      : jmpAddr - size_t(top);
            if (jmpSize == 4)
...
private:
    ...
    static inline uint32_t getPP(uint64_t type) { return (type & T_66) ? 1 : (type & T_F3) ? 2
            : (type & T_F2)                                                                ? 3
                                                                                           : 0; }
    // @@@end of avx_type_def.h
    static inline uint32_t getMap(uint64_t type) { return (type & T_0F) ? 1 : (type & T_0F38) ? 2
            : (type & T_0F3A)                                                                 ? 3
                                                                                              : 0; }
    ...
    int evex(const Reg& reg, const Reg& base, const Operand* v, uint64_t type, int code, const Reg* x = 0, bool b = false, int aaa = 0, uint32_t VL = 0, bool Hi16Vidx = false)
    {
        ...
        if (rounding) {
            ...
            LL = (VL == 512) ? 2 : (VL == 256) ? 1
                                               : 0;
            if (b) {
                disp8N = ((type & T_B16) == T_B16) ? 2 : (type & T_B32) ? 4
                                                                        : 8;
    ...
    void evexLeg(const Reg& r, const Reg& b, const Reg& x, const Reg& v, uint64_t type, int sc = NONE)
    {
        ...
        int V4 = !v.isExtIdx2();
        int ND = (type & T_ZU) ? (r.getZU() || b.getZU()) : (type & T_ND1) ? 1
            : (type & T_APX)                                               ? 0
                                                                           : v.isREG();
    ...
    {
        if (!(type & T_MUST_EVEX) && !d.isREG() && !(d.hasRex2NFZU() || op1.hasRex2NFZU() || op2.hasRex2NFZU()))
            return false;
        const Operand *p1 = &op1, *p2 = &op2;
        if (p1->isMEM()) {
            std::swap(p1, p2);
    ...
    void opRext(const Operand& op, int bit, int ext, uint64_t type, int code, bool disableRex = false, int immSize = 0, const Reg* d = 0)
    {
        ...
        if ((type & T_APX) && op.hasRex2NFZU() && opROO(d ? *d : Reg(0, Operand::REG, opBit), op, r, type, code))
            return;
        if (op.isMEM()) {
    ...
    {
        verifyMemHasSize(op);
        uint32_t immBit = inner::IsInDisp8(imm) ? 8 : isInDisp16(imm) ? 16
                                                                      : 32;
        if (op.isBit(8))
            immBit = 8;
        if (op.getBit() < immBit)
    ...
    */
    ...
    void opFpuMem(const Address& addr, uint8_t m16, uint8_t m32, uint8_t m64, uint8_t ext, uint8_t m64ext)
    {
        if (addr.is64bitDisp())
            XBYAK_THROW(ERR_CANT_USE_64BIT_DISP)
        uint8_t code = addr.isBit(16) ? m16 : addr.isBit(32) ? m32
            : addr.isBit(64)                                 ? m64
                                                             : 0;
        if (!code)
    ...
    {
        uint32_t code = reg1.getIdx() == 0 ? code1 : reg2.getIdx() == 0 ? code2
                                                                        : 0;
        if (!code)
    ...
    {
        if (!(x.isXMM() && op.is(Operand::XMM | Operand::YMM | Operand::MEM) && op.isBit(128 | 256)) && !(x.isYMM() && op.is(Operand::ZMM | Operand::MEM)))
            XBYAK_THROW(ERR_BAD_COMBINATION)
    }
    ...
    void opCvt5(const Xmm& x, const Operand& op, uint64_t type, int code)
    {
        if (!(x.isXMM() && op.isBit(128 | 256 | 512)))
            XBYAK_THROW(ERR_BAD_COMBINATION)
        Operand::Kind kind = op.isBit(128) ? Operand::XMM : op.isBit(256) ? Operand::YMM
                                                                          : Operand::ZMM;
        opVex(x.copyAndSetKind(kind), &xm0, op, type, code);
    }
    ...
    */
    ...
    void opKmov(const Opmask& k, const Operand& op, bool rev, int size)
    {
        ...
        if (rev) {
            code = isReg ? 0x93 : op.isMEM() ? 0x91
                                             : 0;
        } else {
            code = op.isOPMASK() || op.isMEM() ? 0x90 : isReg ? 0x92
                                                              : 0;
        }
        if (code == 0)
            XBYAK_THROW(ERR_BAD_COMBINATION)
    ...
    */
    ...
    void mov(const Operand& op1, const Operand& op2)
    {
        ...
        if (addr && addr->is64bitDisp()) {
            if (code) {
                rex(*reg);
                db(op1.isREG(8) ? 0xA0 : op1.isREG() ? 0xA1
                        : op2.isREG(8)               ? 0xA2
                                                     : 0xA3);
                db(addr->getDisp(), 8);
            } else {
